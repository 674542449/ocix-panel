...
- **同一个账户 + 同一个实例名，正在跑时重复提交会拿回同一个任务号**，
  手快点两下或者被报错吓到去重试，都只会建一台
...
低饱和的**石墨青**：底色带一丝青味（不是蓝黑也不是纯黑），主色是压过饱和度的青，
状态色是鼠尾草绿 / 沙黄 / 陶土玫瑰。

「不刺眼」在数值上就一条：**所有彩色的饱和度都在 50% 以内**。
作为对照，常见的那套 Tailwind 默认色是 71%–93%：

| 用途 | 颜色 | 色相 | 饱和度 |
|---|---|---|---|
| 主色 / 可操作 | `#5fa8b5` | 189° | 37% |
| 运行中 | `#6fae87` | 143° | 28% |
| 需留意 / 过渡中 | `#c9ae5e` | 45° | 50% |
| 出错 / 已终止 | `#d07f8a` | 352° | 46% |

饱和度压下来之后，色相就得拉开——否则两个低饱和色并排摆着会糊成一片。
上面四个两两相邻至少差 40°。

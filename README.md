...
整个面板不用任何彩色，只有中性灰阶（R=G=B）加纯白。

难点不在于把颜色去掉，而在于**颜色原本在传递信息**：绿=运行中、红=已终止、
琥珀=正在变化、青=可操作。全部压成灰之后，这些状态会变成五个一模一样的灰点。
所以每一条信息都换了一个承载通道：

| 原来靠颜色 | 现在靠什么 | 通道 |
|---|---|---|
| 绿 = 运行中 | 实心圆，最亮 | 亮度 |
| 红 = 已终止 | 空心圆环 | 形态（「空」就是停了）|
| 琥珀 = 正在变化 | 实心 + 向外脉动 | 运动（全站唯一会动的东西）|
| 琥珀 = 需留意（账户已升级）| **圆角方块** + 静止外环 | 形状 |
| 青 = 主操作 | 白底黑字反白 | 反相 |
| 青 = 链接 | 白字 + 下划线 | 下划线 |
| 红 = 额度超标 | 斜纹填充 | 纹理 |
| 各类语义标签 | 实线 / 虚线 / 加粗边框 | 线型 |

方块那一档是为关掉动效的人准备的：`prefers-reduced-motion` 下「正在变化」
只剩一个静止的环，如果它和「需留意」同为「实心圆 + 环」就分不出来了。

顺带把无障碍问题修对了。「不要只用颜色传达信息」是条硬规则，彩色版本其实
违反着它（状态只有色相区别）；单色版本被迫给每个状态配上形态，反而合规了。
所有文字对各自背景都过 WCAG AA 4.5:1，最暗的那档微标签也是。

`tests/test_theme.py` 会挡住往回加彩色值：任何 RGB 三通道差超过 6 的颜色
都会让测试失败——包括 favicon。

...
深蓝黑底 + **亮蓝主色 + 紫色副色**，状态色是薄荷绿 / 琥珀 / 珊瑚。

| 用途 | 颜色 | 色相 | HSV 饱和度 |
|---|---|---|---|
| 主色 / 可操作 | `#6f8dff` | 228° | 56% |
| 副色（只跟主色搭）| `#b07cff` | 264° | 51% |
| 运行中 | `#3ddc84` | 147° | 72% |
| 需留意 / 过渡中 | `#ffc75a` | 40° | 65% |
| 出错 / 已终止 | `#ff7d92` | 350° | 51% |

**「亮」不等于「艳」。** 按 HSV 饱和度算，这套是 51~72%，反而**低于**
早期那套被否掉的配色（72~96%）。年轻感来自三处：明度更高、色相更鲜、
以及多了一个副色——单色系怎么调都不会显年轻，一对颜色搭在一起才会。

副色只在进度条、导航选中标记、ARM 机位边框、登录页星球链路上跟主色做渐变，
**不参与状态语义**——绿/黄/红那三档已经排满了，再多一个颜色只会让人误会。

`tests/test_theme.py` 里的饱和度上限是明写的常量（当前 HSV 80%），
方向变了就改那个常量并写清理由，不要在颜色上凑。

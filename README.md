...
- **账户等级**：账户列表直接标出免费号 / 已升级，依据租户订阅记录。
  可以整表重测，也可以在某一行的操作栏点「检测等级」**只查这一个号**——
  整表是串行的（同一时刻只让一个号跟 OCI 通信），号多时要等很久
...
暖中性底盘 + **单一赤陶强调色**，奶油色文字。整体克制、偏纸感，
不是常见的蓝黑控制台。

| 用途 | 颜色 | 色相 | HSV 饱和度 |
|---|---|---|---|
| 强调色 / 可操作 | `#d97757` 赤陶 | 15° | 60% |
| 运行中 | `#6cae7a` 苔绿 | 133° | 38% |
| 需留意 / 过渡中 | `#d9a441` 琥珀 | 39° | 70% |
| 出错 / 已终止 | `#dc7c8b` 玫瑰 | 351° | 44% |

底色是暖炭（R>B），文字是奶油色 `#f0eee6`,不用冷白。**只有一个强调色**,
没有副色——这套风格的性格就是克制，配两个颜色玩渐变就不是那个味道。

### 暖色系带来的一个真问题

赤陶 15° 离「需留意」的琥珀只有 24°、离「出错」的玫瑰 25°。原来的规则是
「四个色两两相邻至少 40°」，这里过不去。但那条规则表达的意图其实不准：

- **三档状态色彼此**必须拉开（它们会同时出现在同一个列表里）——现在
  最小间隔 48°,合格
- **强调色只需离「运行中」远**（踩过：主色曾是青 189°,离绿只有 46°,
  既不像能点又跟状态抢意思）——现在 118°
- 强调色离 warn/crit 近是可以接受的：它永远不以状态点的形态出现，
  而状态点除了颜色还带形态（实心/空心/方块/脉动）

唯一真会混的地方是进度条，所以**普通进度条改成中性色**,警戒和超额才跳得出来。
测试也按这个意图重写了。

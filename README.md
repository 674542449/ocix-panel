...
低饱和的**石墨靛**：冷调石墨底，主色是压过饱和度的靛蓝，
状态色是绿 / 沙黄 / 玫瑰。所有彩色的饱和度都在 57% 以内。

| 用途 | 颜色 | 色相 | 饱和度 |
|---|---|---|---|
| 主色 / 可操作 | `#7f8fd8` | 229° | 53% |
| 运行中 | `#67b98a` | 146° | 37% |
| 需留意 / 过渡中 | `#d4ad63` | 39° | 57% |
| 出错 / 已终止 | `#dd8590` | 352° | 56% |

上一版主色用的是青 `#5fa8b5`（189°）。问题不在饱和度——那已经压到
50% 以内了——在**色相选错了**：青夹在蓝（可操作）和绿（运行中）中间，
离「运行中」的绿只有 46°，既不像「能点」，又跟状态色抢意思。
挪到靛蓝之后离绿 83°，蓝在界面里天然读作「可交互」，也彻底让开了
绿/黄/红那三档状态。

底色仍是冷调石墨——主流暗色界面都这么配，这点本来就没问题，
只是跟着重新调匀了一遍。
